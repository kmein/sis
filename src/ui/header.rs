...
pub fn draw(f: &mut Frame<'_>, area: Rect, app: &App) {
    let theme = &app.theme;
    let [info, hints, logo] = Layout::horizontal([
    ...
    let kv = |k: &str, v: String| {
        Line::from(vec![
            Span::styled(format!("{k:<10}"), theme.info_key),
            Span::styled(v, theme.info_value),
        ])
    };
    let scope = if app.connecting {
        format!("{} (connecting…)", app.scope)
    } else {
        app.scope.to_string()
    };
    ...
    let lines = vec![
        ...
        kv(
            "Units:",
            format!("{} loaded, {} failed", m.n_names, m.n_failed),
        ),
    ...
    for (c, col_area) in col_areas.iter().enumerate() {
        let lines: Vec<Line> = items
            ...
            .map(|(k, l)| {
                Line::from(vec![
                    Span::styled(format!("{k:<9}"), theme.key),
